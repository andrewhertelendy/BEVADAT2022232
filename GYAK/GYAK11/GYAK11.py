...
arr = np.random.rand(5, 5).astype('float32').round()
# print(arr)
...
kernel = np.random.randint(2, size=(3,3)).astype(np.float32)
print(kernel)
...
Padding=0
Stride=1 
out_height=image_height+2*Padding-(kernel_height-Stride)
out_width=image_width+2*Padding-(kernel_height-Stride)
...
output = convolve2d(arr, kernel, mode='valid')
    ...
    def patches_generator(self, image):
        ...
        for h in range(image_h-self.kernel_size+1):
            for w in range(image_w-self.kernel_size+1):
                patch =  image[h:kernel_height, w:kernel_width]
